...
export class CommerceStore {
  ...
  async topUpWallet(userId, amount, metadata = {}) {
    ...
    return this.#transaction(async () => {
      await this.#ensureWalletAccount(userId, now);
      const current = await this.#readWalletRow(userId);
  ...
  async checkout(userId, input = {}) {
    ...
    return this.#transaction(async () => {
      await this.#ensureWalletAccount(userId, createdAt);
      const wallet = await this.#readWalletRow(userId);
  ...
  async #ensureWalletAccount(userId, timestamp = new Date().toISOString()) {
    await this.client.execute({
