...
1. Copy `.env.example` to `.env` and set Redis credentials for persistence.
...
The first request searches Wikidata, stores normalized items for 30 days, and
...
2. Create a Redis database in the Vercel Marketplace or Upstash. Add its REST
   URL and token as `CATALOG_REDIS_REST_URL` and `CATALOG_REDIS_REST_TOKEN` in
   Vercel Project Settings.
3. Deploy. Example: `/api/products/search?q=wireless%20headphones`.

The function-level cache is an optimization only because serverless instances do
not share memory. For reliable cross-instance caching, connect a shared cache
